...
public class ReadHeadersInterceptor extends AbstractSoapInterceptor {
    ...
    private static class HeadersProcessor {
        ...
        private void addEvent(XMLEvent event) {
            try {
                if (!headerFound && event.isStartElement()) {
                    QName qName = event.asStartElement().getName();
                    if (isSoapHeader(qName.getNamespaceURI(), qName.getLocalPart())) {
                        headerFound = true;
                ...
                if (headerFound) {
                    parent = StaxUtils.readDocElement(doc, parent, event, context);
                } else {
                    events.add(event);
                }
            } catch (XMLStreamException e) {
        ...
        private boolean isSoapHeader(String uri, String local) {
            return version.getHeader().getLocalPart().equals(local)
                   && version.getHeader().getNamespaceURI().equals(uri);
        }
    }
}
